...
void MD5Anim::LoadMD5Anim(std::string filename)
{
	...
	*/
	while (!f.eof())
	{
		...
		{	//Actually, there's nothing in this line...usually
...
void MD5Anim::LoadMD5AnimHierarchy(std::ifstream &from, unsigned int &count)
{
	...
	*/
	...
	do
	{
		...
		if (tempLine == "{")
		{			//In a well-behaved MD5 file, the first line will be '{'
		}
		else if (tempLine[0] == '"')
		{	//It's a joint!
