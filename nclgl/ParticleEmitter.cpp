...
ParticleEmitter::ParticleEmitter(void)
{
	...
	// Each particle is a white dot, which has an alpha fade on it, so the edges fade to 0.0 alpha.
	texture = SOIL_load_OGL_texture(TEXTUREDIR"particle.tga", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_COMPRESS_TO_DXT);
}

// In our destructor, we must make sure we delete the particles held in BOTH of our lists!
ParticleEmitter::~ParticleEmitter(void)
{
...
void ParticleEmitter::Update(float msec)
{
	...
	// Now for the particle 'think' function. Particles are so 'lightweight' there's not much point putting this as a member variable of the Particle struct...
...
// This function gets a pointer to an unused particle - either from the freelist, or from a newly created particle on the heap.
Particle* ParticleEmitter::GetFreeParticle()
{
	...
	p->colour = Vector4(1.0f, 1.0f, 1.0f, 1.0f); // White colour
...
// If we end up with more particles than space in graphics memory, we must allocate some new graphics memory!
