...
bool	OBJMesh::LoadOBJMesh(std::string filename)
{
	...
	*/
	...
	while (!f.eof())
	{
		...
		else if (currentLine == OBJMESH || currentLine == OBJOBJECT)
...
void	OBJMesh::SetTexturesFromMTL(string &mtlFile, string &mtlType)
{
	...
	while (!f.eof())
	{
		...
		else if (currentLine == MTLDIFFUSEMAP)
		{
			...
			{
#ifndef DEBUG
				if (mtlFile == ROCK_MTL_FILE_NAME)
				{
					currentMTL.diffuseNum = SOIL_load_OGL_texture(string(TEXTUREDIR + currentMTL.diffuse).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS | SOIL_FLAG_COMPRESS_TO_DXT | SOIL_FLAG_MIPMAPS);
				}
				else
				{
#endif //DEBUG
					currentMTL.diffuseNum = SOIL_load_OGL_texture(string(TEXTUREDIR + currentMTL.diffuse).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS | SOIL_FLAG_COMPRESS_TO_DXT);
#ifndef DEBUG
				}
#endif //DEBUG
			}
		...
		else if (currentLine == MTLBUMPMAP || currentLine == MTLBUMPMAPALT)
		{
			...
			if (!currentMTL.bump.empty())
			{
				...
				if (mtlFile == ROCK_MTL_FILE_NAME)
				{
					currentMTL.bumpNum = SOIL_load_OGL_texture(string(TEXTUREDIR + currentMTL.bump).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS | SOIL_FLAG_COMPRESS_TO_DXT | SOIL_FLAG_MIPMAPS);
				}
				else
				{
#endif //DEBUG
					currentMTL.bumpNum = SOIL_load_OGL_texture(string(TEXTUREDIR + currentMTL.bump).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS | SOIL_FLAG_COMPRESS_TO_DXT);
#ifndef DEBUG
				}
#endif //DEBUG
			}
